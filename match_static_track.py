...
if __name__ == '__main__':
    ...
    data = pd.read_json(args.track_file)
    gps_df = pd.DataFrame(data)
    gps_df['time'] = pd.to_datetime(gps_df['time'])
    gps_df['time'] = gps_df['time'].astype('int64') // 10 ** 9
    gps_df['geometry'] = [Point(xy) for xy in zip(gps_df['lng'], gps_df['lat'])]
    gps_df = gpd.GeoDataFrame(gps_df, geometry='geometry', crs='EPSG:4326')
    output_dir = Path(args.output_fldr)
    output_dir.mkdir(parents=True, exist_ok=True)
    gps_shp_path = output_dir / 'gps.shp'
    gps_df.to_file(gps_shp_path, encoding='utf-8')
